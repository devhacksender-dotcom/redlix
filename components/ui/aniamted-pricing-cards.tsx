...
export const PricingWrapper: React.FC<{
  ...
  className?: string
  buttonText?: string
}> = ({ children, contactHref, className, type = 'waves', buttonText = 'contact' }) => (
  <article
    className={cn(
  ...
  >
    <span
      className={
        'w-full h-full absolute top-0 left-0 z-[2] p-5 sm:p-6 flex flex-col items-start justify-start sm:gap-4 gap-3'
      }
    >
      {children}
      <div className={'w-full flex items-end justify-end text-base mt-auto'}>
        <Link href={contactHref} className={'w-full h-fit relative z-10'}>
          <button className={'h-12 w-full bg-white hover:bg-zinc-100 rounded-lg text-neutral-900 font-bold cursor-pointer transition-colors duration-200 shadow-md flex items-center justify-center gap-2'}>
            {buttonText}
          </button>
        </Link>
      </div>
